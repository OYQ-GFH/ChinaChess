...
class Game(object):
    ...
    def operate(self, flag, coord, old_coord, new_coord, un1_coord, un2_coord, eat, name1, name2):
        ...
        def blit(n):
            ...
            one_coord = None
            two_coord = None
            for code_x_key, code_x_value in CODE_X.items():
                if old_coord[0] == code_x_value:
                    one_coord = code_x_key
                if coord[0][0] == code_x_value:
                    two_coord = code_x_key
            for code_y_key, code_y_value in CODE_Y.items():
                if old_coord[1] == code_y_value:
                    one_coord += code_y_key
                if coord[0][1] == code_y_value:
                    two_coord += code_y_key

            self.code = (one_coord + two_coord).upper()
            search_time_ms = 5000
            pos = Position()
            ...
            if len(user_step) == 4:
                ...
                try:
                    ...
                    self.ai = move2Iccs(mov).replace("-", "").lower()
                    print(self.ai)
                    if self.black:
                        three_coord_x = None
                        three_coord_y = None
                        ...
                        four_coord_y = None
                        for code_x_key, code_x_value in CODE_X.items():
                            if self.ai[0] == code_x_key:
                                three_coord_x = code_x_value
                            if self.ai[2] == code_x_key:
                                four_coord_x = code_x_value
                        for code_y_key, code_y_value in CODE_Y.items():
                            if self.ai[1] == code_y_key:
                                three_coord_y += code_y_value
                            if self.ai[3] == code_y_key:
                                four_coord_y += code_y_value
                        self.ai_coord1 = three_coord_x, three_coord_y
                        self.ai_coord2 = four_coord_x, four_coord_y
                    ...
                    if event.button == 1:
                        if self.click1 is None:
                            ...
                            if not self.go:
                                ...
                                self.window.blit(self.b_chess[0], self.ai_coord2)
                            self.update()
